...
internal/
  ...
  core/
    ...
    autosearch/        Automatic release search + grab pipeline
    blocklist/         Release blocklist management
    collection/        TMDB collection browsing + tracking
    customformat/      Regex-based custom format matching + scoring
    downloader/        Download client config + grab dispatch
    downloadhandling/  Download handling settings (check interval, etc.)
    health/            System health checks (disk, clients, indexers)
    importer/          File import after download completes
    importlist/        Auto-add movies from external lists (TMDB, Trakt, Plex, etc.)
    indexer/           Indexer config + search orchestration
    library/           Library CRUD + disk stats
    mediainfo/         ffprobe-based media scanning
    mediamanagement/   Rename templates + file management settings
    ...
    quality/           Quality profile CRUD + quality definitions
    queue/             Download queue polling
    seedenforcer/      Post-import seed limit enforcement
    stats/             Library statistics + storage snapshots
    tag/               Movie tagging for organization + filtering
  ...
  parser/              Unified release name parser (tokenizer-first architecture)
  plexsync/            Bidirectional library sync (media server ↔ Luminarr)
  radarrimport/        One-time Radarr migration client + orchestrator
  ratelimit/           Per-key token bucket rate limiters
  registry/            Plugin registry (indexers, downloaders, notifiers, media servers, import lists)
  safedialer/          SSRF-protected HTTP transports
  scheduler/           Background job scheduler
  scheduler/jobs/      Built-in jobs: rss_sync, queue_poll, library_scan, refresh_metadata, stats_snapshot, import_list_sync
...
plugins/
  ...
  importlists/tmdb_*/   TMDB lists (popular, trending, collections, persons, etc.)
  importlists/trakt_*/  Trakt lists (popular, trending, anticipated, box office)
  importlists/plex_*/   Plex watchlist
  importlists/mdblist/  MDBList integration
  importlists/stevenlu/ Steven Lu's popular movies
  importlists/custom_list/ Custom RSS/JSON lists
web/
...
| **Releases** | `GET /api/v1/movies/{id}/releases` · `POST /api/v1/movies/{id}/releases/{guid}/grab` · `GET /api/v1/movies/{id}/releases/explain` · `POST /api/v1/movies/{id}/search` |
| **History** | `GET /api/v1/history` · `GET /api/v1/movies/{id}/history` |
| **Stats** | `GET /api/v1/stats/collection` · `GET /api/v1/stats/quality` · `GET /api/v1/stats/storage` · `GET /api/v1/stats/decades` · `GET /api/v1/stats/genres` · `GET /api/v1/stats/growth` |
| **Custom Formats** | `GET/POST /api/v1/custom-formats` · `GET/PUT/DELETE /api/v1/custom-formats/{id}` |
| **Tags** | `GET/POST /api/v1/tags` · `DELETE /api/v1/tags/{id}` |
| **Collections** | `GET /api/v1/collections` · `GET /api/v1/collections/{id}` |
| **Import Lists** | `GET/POST /api/v1/import-lists` · `GET/PUT/DELETE /api/v1/import-lists/{id}` |
| **AI** | `POST /api/v1/ai/command` · `POST /api/v1/ai/command/confirm` |
| **Backup** | `GET /api/v1/system/backup` · `POST /api/v1/system/restore` |
...
- `internal/core/autosearch` — auto-search pipeline, blocklist filtering, quality profile enforcement
- `internal/core/customformat` — regex matching, scoring, TRaSH preset import
- `internal/core/notification` — config CRUD, event dispatch
- `internal/core/stats` — collection stats, quality distribution, storage
...
- `internal/parser` — unified release name tokenizer and quality/audio/edition/group extraction
...
- `internal/api` — full HTTP stack integration tests (real SQLite, real services)
